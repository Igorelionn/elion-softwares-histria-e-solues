...
function TextGradientScroll({
  ...
  return (
    <TextGradientScrollContext.Provider value={{ textOpacity, type }}>
      <p ref={ref} className={cn("relative flex m-0 flex-wrap", className)} style={{ position: 'relative' }}>
        {words.map((word, i) => {
...
const Word = ({ children, progress, range }: WordType) => {
  ...
  return (
    <span className="relative me-2 mt-2">
      ...
      <motion.span style={{ transition: "all .5s", opacity: isRevealed ? 1 : opacity }}>
        {children}
      </motion.span>
    </span>
  );
